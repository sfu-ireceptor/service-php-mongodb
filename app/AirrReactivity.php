...
class AirrReactivity extends Model
{
    ...
    {
        if (isset($_ENV['DB_REACTIVITY_COLLECTION'])) {
            $this->collection = $_ENV['DB_REACTIVITY_COLLECTION'];
        } else {
            $this->collection = 'reactivity';
        }
    ...
    public static function airrReactivityRequest($params)
    {
    ...
    public static function airrReactivityDistinctResponse($response_list)
    {
    ...
    public static function airrReactivityResponse($response_list, $response_type, $params)
    {
        ...
        foreach ($response_list as $reactivity) {
            $return_array = [];
            ...
            foreach ($reactivity as $return_key => $return_element) {
                ...
                if (isset($repository_to_airr[$return_key]) && $repository_to_airr[$return_key] != '') {
                    ...
                    if ($service_name == 'ir_annotation_set_metadata_id_reactivity') {
    ...
    public static function airrReactivityResponseSingle($reactivity)
    {
    ...
    public static function airrOptimizedReactivityRequest($request)
    {
        ...
        if (isset($filter) && $filter != '') {
            AirrUtils::optimizeReactivityFilter($filter, $airr_to_repository_mapping, $airr_types, $service_to_airr_mapping, $service_to_db_mapping, $sample_id_list, $db_filters, $db_types);
        }
        ...
        if ($facets == $service_to_airr_mapping['repertoire_id']) {
            ...
            foreach ($sample_id_query_results_list as $current_repertoire_id => $current_sample_id) {
                ...
                foreach ($current_sample_id as $current_ir_annotation_set_metadata_id) {
                    $db_filters[$service_to_db_mapping['ir_annotation_set_metadata_id_reactivity']] = $current_ir_annotation_set_metadata_id;
            ...
            header('Content-Type: application/json; charset=utf-8');
            $response = AirrUtils::airrHeader('Reactivity', true);
            $response['Facet'] = self::airrReactivityFacetsResponse($return_list);
            ...
            foreach ($sample_id_query_results_list as $current_repertoire_id => $current_sample_id) {
                foreach ($current_sample_id as $current_ir_annotation_set_metadata_id) {
                    ...
                    foreach ($result as $row) {
                        ...
                        $return_array = AirrUtils::convertDbToAirr($reactivity_list, $db_to_airr_mapping, $db_to_service_mapping, $airr_type, $fields_to_display, $response_type, isset($request['include_fields']));
